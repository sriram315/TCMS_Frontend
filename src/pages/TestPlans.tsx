...
const TestPlanRuns: React.FC = () => {
      ...
      try {
        ...
        if (currentUser) {
          if (currentUser.role === "SuperAdmin") {
            ...
            filteredTestPlans = data.filter((plan) => plan.createdBy && plan.createdBy._id === currentUser._id);
          } else {
            ...
            filteredTestPlans = data.filter((plan) => {
              ...
              return plan.testRun.some((run) => run.assignedTo === currentUser._id);
            });
          }
        }
  ...
  const totalProgressData = testPlan.map((_, index) => {
    ...
    return {
      totalCases,
      completedCases,
      progress: totalCases > 0 ? Math.trunc((completedCases / totalCases) * 100) : 0,
    };
  });
        ...
        actions={
          <div>
            {role.toLowerCase() === "admin" && (
              <button type="button" className="btn btn-primary bg-indigo-900" onClick={() => navigate("testPlanForm")}>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
              ...
              <div className="p-5">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-lg font-medium text-gray-900 truncate">
                    <Link state={plan} to={`/test-plans/${plan._id}`} className="hover:text-primary-600 font-semibold">
                      {plan.name}
                    </Link>
                  </h3>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center text-gray-500">
                      ...
                      {totalProgressData[index].totalCases <= 1 ? " test case" : " test cases"}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center text-sm">
                    ...
                    <span className="font-medium text-gray-700">
                      {totalProgressData[index].progress}% ({totalProgressData[index].completedCases}/
                      {totalProgressData[index].totalCases})
                    </span>
                  </div>
              ...
              <div className="px-5 py-3 bg-gray-50 border-t border-gray-200">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-500">
                    Created: {plan?.createdAt && format(new Date(plan?.createdAt), "MMMM dd, yyyy")}
                  </span>
                  <Link
                    state={plan}
        ...
        {testPlan.length === 0 && (
          <div className="flex mb-8 justify-center">
            <p className="text-gray-500">No test plans found matching your criteria.</p>
          </div>
        )}
      </div>
    </div>
