...
//     fetchData();
//   }, [currentUser]);
//   const allTestCasesWithModule = testPlan.map((plan) =>
//     plan?.testRun
...
//     allTestCase.filter((testCase) => testCase.status !== "untested")
//   );
//   const totalProgressData = testPlan.map((_, index) => ({
//     length: allTestCasesWithModule[index].length,
//     progress:
//       progressData[index].length > 0
//         ? Math.trunc(
//             (progressData[index].length /
//               allTestCasesWithModule[index].length) *
//               100
//           )
//         : "0",
//   }));
...
//   return (
//     <div>
//       {/* <Breadcrumbs items={[{ label: "Test Plans", path: "/test-plans" }, { label: "E-commerce Platform" }]} /> */}
...
//                       {totalProgressData[index].length}
//                       {totalProgressData[index].length <= 1
//                         ? "test case"
//                         : "test cases"}
//                     </div>
//                   </div>
...
const TestPlanRuns: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/test-plan");
        const data = response.data.data;
